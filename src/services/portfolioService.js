...
export function analyzeHolding(holding, market) {
  if (!market) {
    return {
      ...
      Fundamental_RRR_Note: "No market data found, so fundamental RRR cannot be calculated.",
...
export async function writePortfolioReport(rows, outputFile) {
  ...
  guideSheet.addRows([
    ...
    {
      field: "Technical_RRR",
      meaning: "RRR จากกรอบเทคนิคเดิม เช่น 52W high/low, stop loss และ exit zone",
      criteria: "Phase นี้ยังเท่ากับ RRR เดิม เพื่อค่อยๆ แยกความหมาย",
    },
    {
      field: "Fundamental_RRR_Status",
      meaning: "สถานะการคำนวณ RRR จากมูลค่าพื้นฐาน",
      criteria: "INSUFFICIENT_DATA = ยังไม่มี fair value/EPS/analyst target เพียงพอ ห้ามตีความว่าเป็น 0 หรือไม่ดี",
    },
    {
      field: "Data_Status",
